...
/// 把 Footer 放进列表内部（通常作为最后一项）。
///
/// Footer 的 [FastRefreshIndicator.position] 需为
/// [FastRefreshIndicatorPosition.locator] 或 `custom`。
class FastFooterLocator extends StatelessWidget {
  /// 是否作为 Sliver 使用。
  ...
  Widget build(BuildContext context) {
    final footerNotifier = FastRefresh.of(context).footerNotifier;
    assert(
        footerNotifier.iPosition == FastRefreshIndicatorPosition.locator ||
            footerNotifier.iPosition == FastRefreshIndicatorPosition.custom,
        'Cannot use FastFooterLocator when header position is not FastRefreshIndicatorPosition.locator.');
    return ValueListenableBuilder(
      ...
      builder: (ctx, notifier, _) {
        ...
        return _FastFooterLocatorRenderWidget(
          isSliver: _isSliver,
...
class _FastFooterLocatorRenderWidget extends SingleChildRenderObjectWidget {
  ...
  const _FastFooterLocatorRenderWidget({
  ...
  RenderObject createRenderObject(BuildContext context) => isSliver
      ? _FastFooterLocatorRenderSliver(
          context: context,
          paintExtent: paintExtent,
        )
      : _FastFooterLocatorRenderBox(
          context: context,
          paintExtent: paintExtent,
        );
}

/// Box 布局下的 Footer 定位：把指示器画在列表下方外侧。
class _FastFooterLocatorRenderBox extends RenderProxyBox {
  ...
  /// 按当前越界偏移把 Footer 画到列表下方 / 右侧外侧。
  @override
  void paint(PaintingContext context, Offset offset) {
    final footerNotifier = FastRefresh.of(this.context).footerNotifier;
    final axis = footerNotifier.axis;
    final axisDirection = footerNotifier.axisDirection;
...
/// Sliver 布局下的 Footer 定位。
class _FastFooterLocatorRenderSliver extends RenderSliverSingleBoxAdapter {
  /// 用于读取 [FastRefreshData]。
  final BuildContext context;
  ...
  /// scrollExtent 为 0，把 Footer 画到视口外侧，不占列表滚动范围。
  @override
  void performLayout() {
    if (child == null) {
